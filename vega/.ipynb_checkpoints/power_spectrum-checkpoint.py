    ...
    def compute(self, pk_lin, params, fast_metals=False):
        ...
        if params["debugging"]:
            if self._tracer1['name'] == self._tracer2['name']:
                print('########### AUTO -- in power_spectrum.py properties for', self._tracer1['name'])
                print('                    are: bias = %s' %bias1 + ', beta = %s' %beta1 )
            else:
                print('########### AUTO+CROSS -- in power_spectrum.py properties for', self._tracer1['name'])
                print('                    are: bias = %s' %bias1 + ', beta = %s' %beta1 )
                print('                    and for ', self._tracer2['name'])
                print('                    are: bias = %s' %bias2 + ', beta = %s' %beta2 )
        ###############
