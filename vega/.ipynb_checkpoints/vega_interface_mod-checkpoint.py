...
class VegaInterface:
    ...
    ### FGmod ###
    def save_sample(self, sample, filename):
        
        if os.path.isfile(filename):
            data_dict = np.load('{}'.format(filename),allow_pickle='TRUE').item()
        else:
            data_dict = {}
            for item in sample:
                data_dict[item] = []
        for item in data_dict:
            data_dict[item].append(sample[item])
    ...
    def chi2(self, params=None, direct_pk=None):
        ...
        for name, corr_item in self.corr_items.items():
            ...
            if self.models.get(name) is None:
                self.models[name] = Model(corr_item, self.fiducial, self.scale_params, self.data[name])
        ##################    
        ...
        ### FGmod ###
        if 'plot_anisotropic_xi' in params.keys():
            if params['plot_anisotropic_xi'] is not None:
                print('Saving anisotropic correlation function')
                self.save_sample(sample={'H0':[params['H0']],\
                                          'omch2':[np.round(params['omch2'], 4)],\
                                          'ombh2':[np.round(params['ombh2'], 4)],\
                                          'ns':[np.round(params['ns'], 4)],\
                                          'As':[params['As']],\
                                          '\ap_{full}':[np.round(params['ap_full'], 4)],\
                                          '\at_{full}':[np.round(params['at_full'], 4)],\
                                          '\beta_{LYA}':[np.round(params['beta_LYA'], 4)],\
                                          '\bias_{LYA}':[np.round(params['bias_LYA'], 4)],\
                                          'cf': [model_cf.tolist()]}, 
                                  filename=params['plot_anisotropic_xi'])
        ############        
    ...
    def log_lik(self, params=None, direct_pk=None):
        ...
        #print(self.priors.items())
        #for param, prior in self.priors.items():
        #    print(param,prior)
        #    log_lik += self._gaussian_lik_prior(prior[1])

        ### FGmod ###
        #print('Vega loglik: ', log_lik)
        #############
